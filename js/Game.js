class Game {
  ...
  start() {
    ...
    Boy1 = createSprite(width / 2 - 50, height - 100);
    Boy1.addImage("boy1", boy1_img);
    Boy1.scale = 0.6;

    //Boy1.addImage("blast", blastImage); //C42 //SA

    Boy2 = createSprite(width / 2 + 100, height - 100);
    Boy2.addImage("boy2", boy2_img);
    Boy2.scale = 0.6;

   // Boy2.addImage("blast", blastImage); //C42//SA
    ...
    if (allPlayers !== undefined) {
      ...
      for (var plr in allPlayers) {
        ...
        if (index === player.index) {
          ...
          camera.position.y = boys[index - 1].position.y;
        }
      }
  ...
  handleFuel(index) {
    // Adding fuel
    boys[index - 1].overlap(fuels, function(collector, collected) {
      player.fuel = 185;
